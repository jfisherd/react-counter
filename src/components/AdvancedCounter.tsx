...
export const AdvancedCounter: React.FC = (): ReactNode => {
    ...
    const handleDecrement = () => {
        ...
        setCount((prevCount) => {
            prevCountToBePassed = prevCount
            return prevCount - stepValue
        })
        setCountHistory((prevCountHistory) => {
            prevCountHistory.push(<li className="decrementRecord">({stepValue}) subtracted from Count ({prevCountToBePassed}) = {count}</li>); // keys not needed? Render OK. Console only noted keyless <li> elements on one occassion. JF 12/15/2025
    ...
    const handleIncrement = () => {
        ...
        setCountHistory((prevCountHistory) => {
            prevCountHistory.push(<li className="incrementRecord">({stepValue}) added to Count ({prevCountToBePassed}) = {count}</li>); // .push modifies an array then returns the length of the new array.
    ...
    const handleReset = () => { // Clear count history and reset count to 0
        setCount(0)
        setCountHistory([<li key={'019b2503-007d-74f3-a6f4-c19b6a898321'}>Reset button clicked</li>])
    }
